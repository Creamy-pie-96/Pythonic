...
## Unified RenderConfig API (Recommended)

### Overview

The new unified API uses a single `RenderConfig` struct to configure all printing and export parameters. This is the **recommended approach** for all new code as it's cleaner, more maintainable, and provides sensible defaults.
...
struct RenderConfig {
    ...
    Format format = Format::text;           // Output format for export
    ...
    bool grayscale_dots = false;            // Use grayscale ANSI colors for dots

    // Color options
    bool invert = false;                    // Invert colors
    ...
    char pause_key = 'p';                   // Key to pause/resume
    char stop_key = 's';                    // Key to stop
    ...
    // Convenience methods
    RenderConfig& with_audio();        // Enable audio
    RenderConfig& interactive();       // Enable keyboard controls
};
```

### Dithering Enum

Use the `Dithering` enum to control grayscale rendering quality:

| Dithering                    | Description                                                  |
| ---------------------------- | ------------------------------------------------------------ |
| `Dithering::none`            | Simple threshold (default) - pixels either on or off         |
| `Dithering::ordered`         | Ordered dithering - smooth gradients, fast, stable for video |
| `Dithering::floyd_steinberg` | Floyd-Steinberg error diffusion - best quality, slower       |
...
| `Mode::colored`       | True color (24-bit ANSI) using half-block characters (1×2 pixels per char)     |
| `Mode::colored_dot`   | True color using braille characters with averaged colors (2×4 pixels per char) |
| `Mode::bw_dithered`   | BW braille with ordered dithering for smooth grayscale gradients               |
| `Mode::grayscale_dot` | Grayscale-colored braille dots with dithering for best quality                 |

### Print with RenderConfig
...
int main() {
    ...
    print("image.png");
    print("image.png", RenderConfig());  // Equivalent

    // With custom mode
    print("photo.png", RenderConfig().set_mode(Mode::colored));

    // With dithering for smooth grayscale
    print("photo.jpg", RenderConfig()
        .set_mode(Mode::bw_dithered)
        .set_max_width(120));

    // Grayscale dots with ordered dithering
    print("photo.jpg", RenderConfig()
...
int main() {
    ...
    // Export with dithering for better quality
    export_media("video.mp4", "smooth", RenderConfig()
        .set_format(Format::video)
        .set_mode(Mode::bw_dithered)
        .set_dithering(Dithering::ordered));

    // GPU-accelerated export (default when available)
    export_media("video.mp4", "fast", RenderConfig()
        .set_format(Format::video)
        .set_gpu(true));

    // CPU-only export (for compatibility)
    export_media("video.mp4", "compatible", RenderConfig()
        .set_format(Format::video)
        .set_gpu(false));
