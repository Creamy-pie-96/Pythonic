...
void benchmark_arithmetic_operations() {
    ...
    // Integer multiplication
    {
        ...
        for (size_t i = 0; i < ITERATIONS; ++i) {
            var_prod = var_prod * 2;
            if (var_prod.get<int>() > 1000000) var_prod = 1;
        }
        end = high_resolution_clock::now();
        double pythonic_time = duration<double, std::milli>(end - start).count();
...
void benchmark_container_operations() {
    ...
    {
        std::vector<int> vec;
        for (size_t i = 0; i < CONTAINER_SIZE; ++i) vec.push_back(i);
        ...
        var lst = list();
        for (size_t i = 0; i < CONTAINER_SIZE; ++i) lst.append(var(i));
...
void benchmark_container_operators() {
    ...
    {
        std::set<int> s1, s2;
        for (size_t i = 0; i < 100; ++i) s1.insert(i);
        for (size_t i = 50; i < 150; ++i) s2.insert(i);
        ...
        for (size_t i = 0; i < 100; ++i) set1.add(var(i));
        for (size_t i = 50; i < 150; ++i) set2.add(var(i));
    ...
    // List concatenation
    {
        ...
        for (size_t i = 0; i < 100; ++i) v1.push_back(i);
        ...
        for (size_t i = 0; i < 100; ++i) list1.append(var(i));
        for (size_t i = 0; i < 100; ++i) list2.append(var(i));
...
void generate_markdown_report(const std::string& filename) {
    ...
    auto best = std::min_element(results.begin(), results.end(), 
        [](const BenchmarkResult& a, const BenchmarkResult& b) { return a.slowdown_factor < b.slowdown_factor; });
    ...
    if (python_count > 0) {
        auto best_py = std::min_element(results.begin(), results.end(), 
            [](const BenchmarkResult& a, const BenchmarkResult& b) { 
                if (a.python_time_ms == 0) return false;
                if (b.python_time_ms == 0) return true;
                return a.pythonic_vs_python < b.pythonic_vs_python; 
            });
        auto worst_py = std::max_element(results.begin(), results.end(), 
            [](const BenchmarkResult& a, const BenchmarkResult& b) { 
                if (a.python_time_ms == 0) return true;
                if (b.python_time_ms == 0) return false;
                return a.pythonic_vs_python < b.pythonic_vs_python; 
            });
