...
namespace pythonic
{
    namespace viewer
    {
        ...
        public:
            ...
            bool dragging_graph_ = false; // Drag entire graph via center handle
            ...
            void sync_from_graph()
            {
                ...
                std::uniform_real_distribution<float> dist_x(100.0f, 700.0f);
                std::uniform_real_distribution<float> dist_y(100.0f, 500.0f);

                for (size_t i = 0; i < n; ++i)
                {
                    NodeState ns;
                    ns.node_id = i;
                    ns.x = ns.pinned_x = dist_x(rng_);
                    ns.y = ns.pinned_y = dist_y(rng_);

                    // Try to get node label/metadata
                    try
                    {
            ...
            void update_physics(float dt)
            {
                ...
                for (auto& node : nodes)
                {
                    ...
                    if (node.activation < 0.01f) node.activation = 0.0f;
                }
            ...
            void update_signals(float dt)
            {
                ...
                // Propagate from arrived signals
            ...
            void render()
            {
                ...
                for (const auto& edge : snapshot.edges)
                {
                    ...
                    if (edge.directed)
                    {
                        ...
                        if (len > 1.0f)
                        {
                            ...
                            float ax = p1.x + dx * len * 0.85f; // Closer to target
                            float ay = p1.y + dy * len * 0.85f;
                ...
                {
                    if (sig.from >= snapshot.nodes.size() || sig.to >= snapshot.nodes.size()) continue;
                ...
                for (const auto& node : snapshot.nodes)
                {
                    ...
                    if (node.is_selected) fill_col = IM_COL32(50, 180, 50, 255);
                    else if (node.is_hovered) fill_col = IM_COL32(180, 100, 220, 255);
                    else fill_col = IM_COL32(80, 80, 100, 255);
            ...
            void render_ui()
            {
                ...
                // Stats
                ImGui::Text("Nodes: %zu", back_snapshot_.node_count);
                ImGui::Text("Edges: %zu", back_snapshot_.edge_count);
                ...
                if (mode_ == ViewerMode::EDIT)
                {
                    ...
                    ImGui::Text("Double-click: Add node");
                    ImGui::Text("Drag node to node: Add edge");
                    ImGui::Text("Select + Delete: Remove");

                    if (ImGui::Button("Add Node Here"))
                    {
                        sidebar_open_ = true;
                    }

                    if (ImGui::Button("Relayout (Topological Sort)"))
                    {
                ...
                // Node info popup on hover
                if (back_snapshot_.hovered_node >= 0 && (size_t)back_snapshot_.hovered_node < back_snapshot_.nodes.size())
                {
                    const auto& node = back_snapshot_.nodes[back_snapshot_.hovered_node];
                    ImGui::BeginTooltip();
                    ImGui::Text("Node %zu", node.node_id);
                    if (!node.metadata_str.empty())
                    {
                        ImGui::Separator();
                        ImGui::TextWrapped("%s", node.metadata_str.c_str());
                    }
                    ImGui::EndTooltip();
                }
            }
            ...
            void add_node_at_center()
            {
                ...
                ns.x = ns.pinned_x = camera_x_ + (std::rand() % 100 - 50);
                ns.y = ns.pinned_y = camera_y_ + (std::rand() % 100 - 50);
            ...
            {
                // Add to actual graph (directed by default in edit mode)
                graph_var_.add_edge(from, to, 1.0, 0.0, true);
            ...
            void delete_selected_node()
            {
...

                {
                    ...
                    // We can't actually remove the node from the graph easily
                    // Mark it as hidden by setting its position far away
            ...
            void do_topological_relayout()
            {
                ...
                try
                {
                    ...
                    // Relayout nodes in topological order
                    ...
                    for (size_t i = 0; i < topo.len(); ++i)
                    {
                        ...
                        if (node_id < back_snapshot_.nodes.size())
                        {
                            float new_x = start_x + i * spacing_x;
                            float new_y = center_y + (std::rand() % 80 - 40);
            ...
            void handle_input()
            {
                ...
                {
                    std::lock_guard<std::mutex> lock(snapshot_mutex_);
                    for (size_t i = 0; i < back_snapshot_.nodes.size(); ++i)
                    {
                        auto& node = back_snapshot_.nodes[i];
                ...
                if (ImGui::IsMouseClicked(ImGuiMouseButton_Left))
                {
                    if (hovered >= 0)
                    {
                        ...
                        // If edit mode, potential edge start
                        if (mode_ == ViewerMode::EDIT) {
                    ...
                    {
                        // Check if hitting center handle
                        // We need the snapshot to calculate center
                        {
                            std::lock_guard<std::mutex> lock(snapshot_mutex_);
                            float sx=0, sy=0; int c=0;
                            for(auto& n : back_snapshot_.nodes) { sx+=n.x; sy+=n.y; c++; }
                            float cx = c>0 ? sx/c : 0;
                            float cy = c>0 ? sy/c : 0;
                            ...
                            if (dist < 20.0f && mode_ == ViewerMode::VIEW) {
                                // Dragging graph means moving all pinned positions
                                dragging_camera_ = true; // Actually we reuse this flag but interpret differently?
                                // Better: create a distinct flag for graph drag
                ...
                if (ImGui::IsMouseDown(ImGuiMouseButton_Left))
                {
                    ...
                    if (dragged_node_ >= 0)
                    {
                        ...
                        if (mode_ == ViewerMode::EDIT) {
                            ...
                            float d = distance(node.x, node.y, 
                                back_snapshot_.nodes[edge_start_node_].pinned_x,
                                back_snapshot_.nodes[edge_start_node_].pinned_y);
                            if (d > 30.0f) creating_edge_ = true;
                        }
                    }
                    else if (dragging_graph_) 
                    {
                        // Move all nodes (pinned + current)
                        std::lock_guard<std::mutex> lock(snapshot_mutex_);
                ...
                if (ImGui::IsMouseReleased(ImGuiMouseButton_Left))
                {
                    if (dragged_node_ >= 0) {
                        ...
                        if (creating_edge_ && hovered >= 0 && hovered != edge_start_node_) {
